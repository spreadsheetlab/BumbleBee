...
namespace BumbleBee.Refactorings
{
    ...
    {
        private InlineFormula inlineFormula = new InlineFormula();

        public override void Refactor(Range toName)
        {
            string subject = (toName.Count > 0) ? "Range" : "Cell";
            string name = Microsoft.VisualBasic.Interaction.InputBox($"{subject} name:", "Cell Name");
        ...
        public void Refactor(Range toName, string name)
        {

            var errors = new List<Exception>();
            foreach (var area in toName.Areas.Cast<Range>())
            {
